...
function ClientsDetails() {
    ...
    useEffect(() => {
        socket.on('client-activity', (param) => {
        ...
        return () => {
            socket.off('client-activity');
        };
    }, []);

    useEffect(() => {
        socket.on('document-activity', (param) => {
          if (param.data?.clientId == id) {
              getClientDocs();
          }
          if(param.data?.clientId?._id == id) {
             getClientDocs();
          }
        });
    ...
    const getClient = async () => {
        ...
        return fetch(`${configData.SERVER_URL}/client/getSingleClient/${id}`, {
            method: "get",
            headers: {
    ...
    const removeClient = async() => {
        if (!window.confirm(`are you sure want to ${data.status == true ? "deactivate" : "activate"} ${data?.fullName}?`)) {
            return
        }
        ...
        try {
            ...
            const response = await fetch(`${configData.SERVER_URL}/client/remove/${id}`, {
              method: "post",
              headers: {
                "x-auth-token": window.localStorage.getItem("token")
              },
        ...
        <>
            ...
            <Offcanvas show={showOffcanvas} onHide={offCanvasClose} placement={'end'}>
                ...
                <Offcanvas.Body>
                    ...
                    <form onSubmit={handleSubmit(updateClient)} className="pt-3" enctype="multipart/form-data" id="submit">
